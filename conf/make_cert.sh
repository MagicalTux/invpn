...
if [ ! -f client1.crt ]; then
	openssl req -config tibanne.cnf -batch -subj "/C=JP/ST=Tokyo/L=Shibuya/O=Tibanne Co. Ltd./OU=VPN/CN=02:50:56:00:00:01" -newkey rsa:4096 -nodes -keyout client1.key -out client1.req
...
if [ ! -f client2.crt ]; then
	openssl req -config tibanne.cnf -batch -subj "/C=JP/ST=Tokyo/L=Shibuya/O=Tibanne Co. Ltd./OU=VPN/CN=02:50:56:00:00:02" -newkey rsa:4096 -nodes -keyout client2.key -out client2.req
